 ...
 *
 * These were written inside `form.test.ts` as a reachability suite, moved to
 * `test/helpers/` the moment Taste Gate #4 needed something to draw, and moved here
 * when the `/styles` page needed the same thing. Only four of the fourteen forms are
 * reachable from the ten real fixtures, so anything built from those would judge
 * four styles and guess at ten.
 *
 * **Why this is `src/` and not `test/`.** It ships. `@kodama/engine/form-cases` is
 * a subpath export, so the site can render a specimen per style without the ten
 * fixtures' coverage hole, and the service - which never imports it - pays nothing
 * for it at cold start. The same argument was already made for `fixtures/`: the
 * taste gate judged these accounts and a public page displays them, so calling them
 * private test scaffolding would be the inaccurate description.
 *
 * Nothing here touches the disk, because `src/` does not (the engine renders inside
 * a Worker). That is why `FORM_CASE_DATE` is a literal below rather than being read
 * from `fixtures/index.json`, and why a test pins the two together.
 *
...
import { FORM_NAMES } from "./form.js";
import type { FormName } from "./form.js";
import type { NormalizedHistory, RepoMix, WeekCell } from "./types.js";
import { historyWith, weeksEndingAt } from "./history-builder.js";

/**
 * The date these accounts are read at - the same one every fixture is authored
 * against. Written out rather than imported, because that value lives in
 * `fixtures/index.json` and `src/` reads no files. `fixtures.test.ts` asserts the
 * two are equal, so this cannot quietly drift away from the fixture anchor.
 */
export const FORM_CASE_DATE = "2026-07-15";
